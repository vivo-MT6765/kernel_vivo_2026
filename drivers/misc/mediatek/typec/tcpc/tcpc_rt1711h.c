...
#undef pr_fmt
#define pr_fmt(fmt) "rt1711h: %s: " fmt, __func__
...
struct rt1711_chip {
	...
	struct delayed_work poll_reg_work;
...
{
	int status, role_ctrl, cc1_role, cc2_role;
	...
	*cc2 = TCPC_V10_REG_CC_STATUS_CC2(status);
	...
	if (act_as_drp) {
		...
		cc1_role =  TCPC_V10_REG_CC_STATUS_CC1(role_ctrl);
		cc2_role =  TCPC_V10_REG_CC_STATUS_CC2(role_ctrl);
		if ((cc1_role == TYPEC_CC_RP) || (cc2_role == TYPEC_CC_RP))
			act_as_sink = false;
		else
			act_as_sink = true;
	}
...
static int rt1711_is_reg_default(struct tcpc_device *tcpc)
{
	int rv = rt1711_i2c_read8(tcpc, RT1711H_REG_DRP_TOGGLE_CYCLE);
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	np = of_find_node_by_name(NULL, "type_c_port0");//of_find_node_by_name(NULL, "rt1711_type_c_port0");
	if (!np) {
...
int vivo_poll_is_reg_default(void)
{
	int ret = 0;
	...
	if (!tcpc) {
		pr_info("get tcpc device type_c_port0 fail\n");
		return -1;
	}

	ret = rt1711_is_reg_default(tcpc);
	if (!ret)
		pr_info("register is not default, no need re_init!\n");
	else {
		ret = rt1711_tcpc_init(tcpc, false); //20190724
		//ret = rt1711_tcpc_init(tcpc, true);
		if (ret < 0) {
			pr_info("tcpci_init fail\n");
		}
	}
	pr_info("\n");
	return 0;
...
static void vivo_poll_is_reg_default_work(struct work_struct *work)
{
	struct rt1711_chip *chip = container_of(
		work, struct rt1711_chip, poll_reg_work.work);
	if(!chip->suspend)
		vivo_poll_is_reg_default();
	schedule_delayed_work(&chip->poll_reg_work, msecs_to_jiffies(10*1000));
}
				...
				const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->poll_reg_work, vivo_poll_is_reg_default_work);
	chip->irq_wake_lock =
	...
	schedule_delayed_work(
		&chip->poll_reg_work, msecs_to_jiffies(30*1000));
	pr_info("%s probe OK!\n", __func__);
	return 0;
...
static void rt1711_shutdown(struct i2c_client *client)
{
	...
	data = RT1711H_REG_BMCIO_LPEN;
	rt1711_reg_write(client, RT1711H_REG_BMC_CTRL, data);
