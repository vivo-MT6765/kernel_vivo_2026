...
static struct device_attribute tcpc_device_attributes[] = {
	...
	TCPC_DEVICE_ATTR(is_reg_default, 0666)
};
				   ...
				   const char *buf, size_t count)
{
	...
	case TCPC_DESC_IS_REG_DEF:
		...
		ret = tcpci_is_reg_default(tcpc);
		if (ret != 1)
			dev_err(dev, "%s: register is not default\n", __func__);
		else if (val == 1) {
			dev_err(dev, "%s: val = %ld\n", __func__, val);
			ret = tcpci_init(tcpc, true);
			if (ret < 0) {
				dev_err(dev, "%s tcpci_init fail\n", __func__);
				return -EINVAL;
			}
		}
		break;
